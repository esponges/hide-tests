...
class HideClient:
    ...
    def get_project(self, project_id: str) -> model.Project:
        ...
        print("Get project response status code:", response.status_code)
        print("Get project response text:", response.text)

        if not response.ok:
            raise HideClientError(f"Failed to get project: {response.text}")

        response_data = response.json()
        print("Get project response JSON:", response_data)
